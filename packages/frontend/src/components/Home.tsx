...
const Home = () => {
    ...
    const myfunc = async () => {
        await ContractInteractor.getInstance().transferToken(destinationAddress, parseInt(transferAmount))
    }
    ...
    return (<div>
        ...
        <button onClick={async () => setBalance(await ContractInteractor.getInstance().getTokenBalance())}>Click here to refresh current balance</button>
        ...
        <br/>
        <h2>Step 3: Grant access to the contract by signing (so that contract can do the transfer through relay)</h2>
        ...
        <form onSubmit={handleSubmit}>
         ...
         <label>
          Amount of dai:
          <input type="text" value={transferAmount} onChange={(event) => setTransferAmount(event.target.value)} />
         </label>
